...
### Fixed
- `whoosh.qparser.dateparse` (`DateParserPlugin`): a colon time-of-day appended to a **month- or year-precision** numeric date is no longer mis-bound as `day + hour` (#196). For example `added:"2026-08 15:00"` used to resolve to `[2026-08-15 00:00, 2026-08-15 01:00)` — reading `15` as the day and `00` as the hour. This was a side effect of the ISO-8601 improvement, whose numeric grammar used a single blanket `[- .:/tT]*` separator between *every* component, letting `:` act as the day boundary. The grammar is now split into a date sub-sequence, a date↔time boundary, and a time sub-sequence, and only the time sub-sequence accepts `:`; such a value now fails to parse and falls through (an error/`_NullQuery` rather than a wrong range), matching upstream Whoosh 2.7.4's non-match. All valid compact/`-`/`/`/`.`/space and ISO `T` date-time forms (`2005051001`, `2005.05.10.01`, `2005 05 10`, `2026-08-10 15:00`, `2023-05-17T14:30`) are preserved. Surfaced by the [`stumpylog/whoosh-compat`](https://github.com/stumpylog/whoosh-compat) divergence corpus (#70); thanks @stumpylog. The related *period + time silently widens* case (`2026 23:59` → whole-year range at 23:59, inherited from upstream 2.x) is tracked separately in #196.
